...
object ServerSecurity {
    ...
    fun encrypt(plain: String): String {
        return try {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, SecretKeySpec("asSa%s|n'$ crEed".toByteArray(StandardCharsets.UTF_8), "AES"), IvParameterSpec(iv))
    ...
    fun decrypt(encoded: String): String {
        return try {
            ...
            val cipher = Cipher.getInstance(KeyProperties.KEY_ALGORITHM_AES + "/" + KeyProperties.BLOCK_MODE_CBC + "/" + "PKCS5Padding")
            cipher.init(Cipher.DECRYPT_MODE, SecretKeySpec("asSa%s|n'$ crEed".toByteArray(StandardCharsets.UTF_8), "AES"), IvParameterSpec(iv))
